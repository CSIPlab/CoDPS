...
class GaussianDiffusion:
    ...
    def p_sample_loop(self,
        ...
        for idx in pbar:
            ...
            with torch.no_grad():
                # Do not create computation graph when passing through the network
            ...
            img, distance = measurement_cond_fn(x_t=out['sample'],
                                      ...
                                      x_prev=img,
                                      x_0_hat=out['pred_xstart'], alpha_cum_prod = self.alphas_cumprod, t = time, alphas =  1.0 - self.betas)
            img = img.detach_()
           
            pbar.set_postfix({'distance': distance.item()}, refresh=False)
            if record:
                if idx % 10 == 0 or (idx < 10):
                    file_path = os.path.join(save_root, f"progress/{fname}/x_{str(idx).zfill(4)}.png")
...
class DDIM(SpacedDiffusion):
    def p_sample(self, model, x, t, eta=1.0):
        ...
        # Equation 12.
        sample = out["pred_xstart"] * torch.sqrt(alpha_bar_prev) + c2 * eps
        
        if t != 0:
            sample += c1 * torch.randn_like(x)
