...
def normalize_np(img):
    ...
    # print("img stat ", img.min().item(), img.max().item())
    # img = img / 2 + 0.5 
    img -= np.min(img)
    img /= np.max(img)
    return img
...
class Blurkernel(nn.Module):
    def __init__(self, blur_type='gaussian', kernel_size=31, std=3.0, device=None):
        ...
        self.std = std
        self.device = device
        if blur_type == 'bicubic':
            self.seq = nn.Sequential(
                # nn.ReflectionPad2d(self.kernel_size//2),
                nn.CircularPad2d(self.kernel_size*2),
                nn.Conv2d(3, 3, self.kernel_size*4, stride=1, padding=0, bias=False, groups=3)
            )
            self.seq_adoint = nn.Sequential(
                # nn.ReflectionPad2d(self.kernel_size//2),
                nn.CircularPad2d(self.kernel_size*2),
                nn.Conv2d(3, 3, self.kernel_size*4, stride=1, padding=0, bias=False, groups=3)
            )
        elif blur_type == 'avg_pool':
            self.seq = nn.Sequential(
                # nn.ReflectionPad2d(self.kernel_size//2),
                nn.CircularPad2d(self.kernel_size//2 ),
    ...
    def weights_init(self):
        if self.blur_type == "gaussian":
            ...
            for name, f in self.named_parameters():
                # print("added to name: ", name)
                f.data.copy_(k)
            # print("GDBR blur kernel  mean ", k.mean(axis=1))
            ...
            for name, f in self.named_parameters():
                f.data.copy_(k)
            # print("motion blur kernel  mean ", k.mean(axis=1))
        elif self.blur_type == "bicubic":
            k = np.zeros((self.kernel_size * 4))
            for i in range(self.kernel_size * 4):
                x = (1/self.kernel_size)*(i- np.floor(self.kernel_size*4/2) +0.5)
            ...
            for _, f in self.named_parameters():
                f.data.copy_(kernel)
        elif self.blur_type == "avg_pool":
            k = np.ones((self.kernel_size , self.kernel_size))
            k = k / np.sum(k)
            kernel = torch.from_numpy(k).float()
            self.k = kernel
            for _, f in self.named_parameters():
